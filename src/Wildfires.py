...
if page == pages[0]:
    ...
    st.image(PATH_TO_ASSET + "/feux_intro.jpg")

    st.write("\n")
    
    st.markdown("Le dataset est une **base de données spatiales des feux de forêt** ayant eu lieu aux **États-Unis** entre **1992**\
                et **2015**. Cette base de données comprend **1.88 million** d'enregistrements d'incendies géoréférencés,\
    ...
    st.markdown("- Comprendre l'évolution des feux de forêt aux États-Unis entre 1992 et 2015 grâce à la **Data Visualisation**.\n"
    ...
    df = pd.read_csv(PATH_TO_DATASET + "/df_head.csv", index_col= 0)
    ...
    df2 = pd.read_csv(PATH_TO_DATASET + "/df_nan.csv")
    ...
    df3 = pd.read_csv(PATH_TO_DATASET + "/df_head_2.csv", index_col= 0)
    ...
    st.image(PATH_TO_ASSET + "/classes_desequilibre.png")
    ...
    st.image(PATH_TO_ASSET + "/matrice_corr.png")
...
if page==pages[2]:
    ...
    with header:
        ...
        st.subheader("Évolution globale sur la période")
        ...
        st.markdown("Les superficies brûlées annuellement ont presque doublées alors que le nombre d'incendies n'a augmenté que de 13 % :\n"
                    "- **1.5M** d’ha brûlés pour **72K** incendies en moyenne dans les années 1990.\n"
        ...
        st.markdown(" Étant donné la taille du territoire étudié, il peut y avoir des **disparités géographiques**, avec une saison des feux différente selon la localisation. Afin de vérifier cette hypothèse, nous regroupons les États selon les régions suivantes : WEST, MIDWEST, NORTHEAST, SOUTH et PACIFIC.")
        ...
        st.markdown("Avec ce découpage géographique, on constate qu'il n'y a qu’**une seule « saison des feux » par région**, mais qu’elle diffère selon la localisation :")
        lst = ["Une saison courte dans les régions MIDWEST et NORTHEAST, de mars à mai ;", "Une saison plus longue en début d'année pour la région SOUTH, de février à avril. On constate de plus qu'il n'y a pas de « basse saison », le nombre de feux le reste de l'année demeurant important, y compris en hiver ;", "Une saison longue dans les régions WEST et PACIFIC, de mai à août / septembre."]
        s = ''
        for i in lst:
            s += "- " + i + "\n"
        st.markdown(s)
            
        st.markdown("Intéressons-nous maintenant aux feux de classes 6 et 7, les feux de taille très importante. Nous voulons voir s'ils surviennent durant ces pics de nombre d’incendies ou si leur distribution est différente :")
        ...
        st.markdown("Fortes disparités géographiques : les États du Sud et l'Ouest apparaissent majoritairement en rouge tandis que l’intérieur des terres semble épargné.")
        ...
        st.markdown("Il existe de grandes disparités entre les États, que ce soit en nombre d'incendies ou en superficie brûlée. Le total des superficies ne semble pas directement corrélé au nombre d’incendies. Voici quelques chiffres marquants illustrant cela :")
        lst = ["l'Alaska totalise à lui seul près de 25 % des superficies brûlées pour moins de 1 % des incendies ;", "la Géorgie, deuxième en nombre total d'incendies avec plus de 9 %, ne représente qu’à peine 1 % des superficies brûlées ;", "la Californie est première en nombre d'incendies (10 % du total) et troisième en forêts brulées avec 9% des superficies."]
        s = ''
        for i in lst:
            s += "- " + i + "\n"
        st.markdown(s)
            
        #st.markdown('En regroupant les données par région, ces disparité géographiques sont très visibles :')
        ...
        #lst = ["La région Pacific, dans laquelle l’Alaska est inclus, n'a qu'un nombre très limité d'incendies sur la période et se positionne en deuxième position des superficies parties en fumée avec 13 millions d’hectares ;","La région South compte le plus d’incendies (près de 1 millions en 24 ans), mais ne totalise « que » 10 millions d’hectares brûlés ;","La région WEST, incluant la Californie qui revient régulièrement à la une des journaux ces dernières années avec des feux hors de contrôle, totalise plus de 30 millions d’hectares brûlées pour 550 000 feux, soit 3 fois plus d’hectares pour 2 fois moins d’incendies que la région SOUTH ;","Les régions MIDWEST et NORTHEST ne sont pas très touchées, que ce soit en nombre d'incendies ou de superficies brûlées."]
        #s = ''
        #for i in lst:
        #    s += "- " + i + "\n"
        #st.markdown(s)
        ...
        #sorted_df = df.sort_values(by=['fire_year'], ascending=True)
        #fig9 = px.scatter_mapbox(sorted_df, 
        #    lat="latitude", 
        #    lon="longitude", 
        ...
        #    hover_data = {"state" : False, "latitude" : False, "longitude" : False},
        #    title = "La cause des feux de forêt entre 1992 et 2015",
        #    labels = {"fire_year" : "Année", "stat_cause_regroup" : "Cause du départ du feu"},
        #    zoom=3)
        ...
        st.markdown("- Les incendies sont causés en très grande majorité par l'activité humaine directe et indirecte (Human et Infrastructure) jusqu'en 2005 ;\n"
                    "- Hausse significative des feux d’origine diverses à partir de cette année (changement dans la méthode de classification, ou de méthodologie ?) ;\n"
        ...
        st.subheader("Analyse de l'impact des température sur le nombre de départs de feux et de leur taille")   
...
if page==pages[3]:
      ...
      st.markdown("Rappel : Nous avons le choix entre un traitement de Machine Learning de type :  \n "+"- **Régression** avec 'fire_size' comme variable cible ;  \n "+"- **Classification** avec 'fire_size_class' comme variable cible, catégories numérotées de 1 à 7.")
      st.write("Préparation du jeu de données :")
      st.image(PATH_TO_ASSET + "/scikitschema2.jpg")
      ...
      st.write("- Résultats en régression :")
      
      st.image(PATH_TO_ASSET + "/regression.jpg")
      st.write("- Non concluants. Résultats en classification :  ")
      st.image(PATH_TO_ASSET + "/classification.jpg")

          
      st.write("- Nous poursuivons donc avec des essais approfondis en **classification** :")
      
      #st.write("- La prédiction s'effectue donc sur **les catégories de taille de feux**, numérotées de 1 à 7.")
      st.write("- Algorithmes testés :  **arbre de décision, SGD, XGBoost, RandomForest**")
      
      st.image(PATH_TO_ASSET + "/DecisionTree1a.jpg") 
      ...
      st.write("-   **Rapport de classification** : Le constat est le même pour l'arbre de décision et pour tous les modèles testés : seules **les deux premières classes ont des résultats corrects**, et les cinq autres classes sont très mal prédites. Cela vient du fort **déséquilibre du jeu de données**. Il faut donc procéder à un rééchantillonnage, et bien tenir compte des métriques pour chaque classe.")

      st.write('-   Les modèles donnant un meilleur score pour le jeu test : **SGD, XGBoost** et **RandomForest**, avec des "test scores" entre 0.61 et 0.63. Ils correspondent tous à des algorithmes d’optimisation.')
     
      st.write('## Seconde étape : optimisation')
      
      st.markdown('### Ajout de données')
      st.write("Les faibles résultats nous ont amenés à enrichir notre dataset de données complémentaires à ce stade.")
      ...
      col1.image(PATH_TO_ASSET + "/forest.jpg")
      ...
      col2.image(PATH_TO_ASSET + "/fire_station.jpg")
      ...
      st.image(PATH_TO_ASSET + "/RF2ag.jpg")
      with st.expander("matrice de confusion"):
          st.image(PATH_TO_ASSET + "/RF2amat.jpg")
      st.image(PATH_TO_ASSET + "/XGBoost2ag.jpg")
      with st.expander("matrice de confusion"):
          st.image(PATH_TO_ASSET + "/XGBoost2amat.jpg")
      
      st.write("-  Après rééchantillonnage, le **score moyen** diminue fortement, passant de 0.62 à 0.39.")
      ...
      st.image(PATH_TO_ASSET + "/XGBoost3opta2.jpg")
      ...
      st.write("- Constat à ce stade : Les résultats sont corrects pour les feux de classes 1, 2 et 7 mais demeurent médiocres pour les classes 3 à 6.")
      ...
      with col1:
          st.write("##### Catégorie 1 ( anc. 1 + 2) :  ")
          st.write("##### < à 0.4 ha = moitié de la surface d’un terrain de football.")
          st.image(PATH_TO_ASSET + "/foot.jpg")
      with col2:
          st.write("##### Catégorie 2 ( anc. 3) : ")
          st.write("##### 0.4 - 40 ha = un cours de golf")
          st.image(PATH_TO_ASSET + "/golf.jpg")
      with col3:
          st.write("##### Catégorie 3 (anc. 4 à 7) : ")
          st.write("##### > à 40 ha")
          st.image(PATH_TO_ASSET + "/image_feu2.jfif")
      ...
      st.write("-  Résultats obtenus avec **XGBoost** et sélection des hyperparamètres :")
      
      st.image(PATH_TO_ASSET + "/cat3xgba.jpg")
      st.write("- Le score moyen a été nettement amélioré, passant de 0.39 à 0.59.")
      st.write("- Pour la catégorie minoritaire, le rappel a pu être augmenté à 0.65.")
     
      st.write ('##### Interprétation')
      st.write("Le modèle XGBoost est très performant mais est difficile à interpréter, souvent considéré comme une boîte noire. Nous allons utiliser le **package Shap** pour tenter de comprendre comment l'algorithme est parvenu à ces conclusions et quelles variables ont eu le plus de poids.")
      
      st.write("######  Les 30 variables les plus importantes pour le modèle XGBoost en fonction des valeurs de Shap :")
      
      st.image(PATH_TO_ASSET + "/shap3cata.jpg")
    ...
    st.image(PATH_TO_ASSET + "/Deerfire.jpg")
